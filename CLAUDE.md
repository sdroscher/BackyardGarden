...
- **Brother Nature scraper uses plain Req** — brothernature.ca previously blocked Erlang's TLS fingerprint (requiring curl workarounds), but as of April 2026 plain `Req.get` works. If Cloudflare blocks resume, Node.js (`System.cmd("node", ...)`) bypasses it; curl does not (no arm64 macOS binary for curl-impersonate).
- **Supplier scraper pattern** — `fetch_all_products/0` returns `{upserted, skipped, errors}` tuple; upserts inline as each product is fetched (not batched at end); skips products already in DB with `care_html`; retries upserts up to 3× on `DBConnection.ConnectionError` with 10/20/30s backoff; sequential HTTP fetches with `Process.sleep(2000)` between products.
- **`idle_interval: 30_000` in Repo config** — prevents TCP connections from being killed when idle (e.g. during long-running scrapes). Already set in `dev.exs` and the prod block of `runtime.exs`.
...
- **`phx-value-value` is broken on `<button>`** — `value` is an HTML IDL attribute; the LiveView JS reads the element's `.value` property (empty string) instead of the `phx-value-value` attribute content. Use any other suffix: `phx-value-pill`, `phx-value-id`, etc.
- **`render_click(view, event, params)` bypasses button attributes** — params are sent directly to the handler, so tests pass even when `phx-value-*` attributes on the actual button are wrong. Use `element(...) |> render_click()` to catch attribute mismatches.
